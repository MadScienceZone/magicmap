    ...
    def _wrapper(*a, **k):
        try:
            ...
            tk.messagebox.showerror(
                ...
                message=f"Something's wrong with the fabric of reality here.\n{problem}"
            )
    ...
    class MousePosition(object):
        ...
        def text(self):
            if (self._point.x < 1000 and self._point.y < 1000):
                return f"x: {self._point.x:6.2f} y: {self._point.y:6.2f}"
            return "x: ------ y: ------"
...
#
    def help(self, *a): 
        tk.messagebox.showinfo(title="Help", message=f'''The documentation for {self.prog_name} is currently online.
Please see https://www.rag.com/tech/tools/{self.prog_name} for full information about this program.''')

    def about(self, *a):
        tk.messagebox.showinfo(title=f"About {self.prog_name}", message=f'''
Ragnarök Magic Mapper %MAGICMAP_RELEASE%
Written by Fizban, based on some original ideas by Klive, and with much help by Polyone.
©1993, 2000, 2001, 2002, 2003, 2010, 2012, 2018, 2020. All Rights Reserved. Distributed under the terms and conditions of the BSD 3-Clause License.
...
class MapClientFrame (MapViewerFrame):
    def __init__(self, master=None, host=None, local_mode=False, proxy_port=None, socks_proxy=None, web_base=None, verbose=0, **k):
        MapViewerFrame.__init__(self, master=master, verbose=verbose, **k)
#        self.creator_re = re.compile(re.escape(os.path.sep)+'players'+re.escape(os.path.sep)+r'(\w+)')
#        self.expand_globs = expand_globs
#        self.file_list = file_list or []
...
#            self.reload()
    ...
    def start_queue_runner(self):
        self.after_idle(self._check_event_queue)

    def _check_event_queue(self):
        if self.event_queue and not self.event_queue.empty():
            try:
                self._handle_incoming_event(self.event_queue.get())
            finally:
                self.after(0, self._check_event_queue)
        else:
            self.after_idle(self._check_event_queue)

    def _handle_incoming_event(self, event):
        "Receive commands from the MUD server and act on them by updating our map display."
